...
# Per-platform canvas fill (fraction the shape spans). macOS Dock icons sit in a
# padded squircle grid, so the glyph must leave native-style padding or it reads
# oversized next to its neighbors. Windows/Linux/in-app icons have no such grid
# and look undersized when padded, so they fill the canvas edge to edge. Keep
# these split — collapsing them to one value is what made the Windows icon shrink.
SQ_FILL_MAC = 0.82   # macOS .icns: native-style padding
SQ_FILL_FULL = 1.0   # Windows .ico, Linux hicolor, runtime window icon: full bleed
...
def masked_master(fill: float) -> Image.Image:
    """Master art clipped to a squircle spanning `fill` of the canvas."""
    art = rasterize_svg(MASTER, BASE)
    mask = squircle_mask(BASE, SQ_N, fill)
    r, g, b, a = art.split()
...
def main() -> None:
    ...
    # Two masters: padded for the macOS Dock grid, full-bleed for everyone else.
    mac = masked_master(SQ_FILL_MAC)
    full = masked_master(SQ_FILL_FULL)

    def at(master: Image.Image, px: int) -> Image.Image:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for px, label in [
            ...
            at(mac, px).save(iconset / f"icon_{label}.png")
        subprocess.run(
    ...
    at(full, 256).save(ICON / "window-256.png")
    # macOS shows the *runtime* window icon in the Dock: eframe calls
    # setApplicationIconImage at launch, which overrides even a bundled .icns. So
    # the macOS runtime icon needs the same padded squircle as the .icns or it
    # reads oversized next to native Dock icons. `app_icon()` picks this on macOS.
    at(mac, 256).save(ICON / "window-256-mac.png")

    print("==> Linux hicolor")
    for px in (16, 22, 24, 32, 48, 64, 128, 256):
        out = ICON / "hicolor" / f"{px}x{px}" / "apps"
        out.mkdir(parents=True, exist_ok=True)
        at(full, px).save(out / "silicolab.png")
