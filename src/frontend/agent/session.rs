...
use crate::io::llm::types::{ChatMessage, ContentBlock, DocumentRef, ToolCall, Usage};
...
/// What the user sent in one turn: typed text plus any attached PDFs.
...
impl UserMessage {
    /// The neutral history message: documents first, so providers that read
    /// content in order see the file before the question about it.
    pub fn to_chat_message(&self) -> ChatMessage {
        ...
        ChatMessage {
            role: crate::io::llm::types::Role::User,
            content,
        }
    }
}
...
impl PendingTurn {
    ...
    pub fn preview(&self) -> &str {
        match self {
            ...
            PendingTurn::UserMessage(message) if message.text.is_empty() => "Attached PDF",
            PendingTurn::UserMessage(message) => &message.text,
...
pub struct AssistantConversation {
    ...
    pub attachments: Vec<DocumentRef>,
    /// The extracted-text explanation is shown once per conversation.
    pub text_fallback_noted: bool,
...
impl AssistantConversation {
    ...
    pub fn resolve_current_backlog(&mut self, jobs_running: usize) {
        if jobs_running > self.backlog_job_baseline {
            self.current_backlog = None;
        } else {
            self.flush_current_backlog();
        }
    }
