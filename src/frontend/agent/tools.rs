    ...
    vec![
        ...
        ToolDef {
            ...
            description: "Read-only look at the current workspace: the active entry, its \
                atom/bond/chain counts, composition and bond geometry, QM convergence and artifact status, MD/QM provenance and \
...
pub fn diagnostic_tool_allowed(name: &str) -> bool {
    matches!(name, "inspect" | "list_jobs" | "recommend_method")
}

pub fn execute_tool(state: &mut AppState, call: &ToolCall) -> ToolOutcome {
    if state.ui.agent.qm_diagnostic_only && !diagnostic_tool_allowed(&call.name) {
        return ToolOutcome {
            content: "QM diagnosis is read only; await a new user instruction.".into(),
            is_error: true,
        };
    }
    match call.name.as_str() {
